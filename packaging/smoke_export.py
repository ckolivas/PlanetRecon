"""Exercise all eight mono/RGB scientific export variants through a frozen executable.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='planetrecon-export-smoke-') as directory:
        ...
        for color in ('mono', 'RGB'):
            ...
            subprocess.run([executable, '--threads', '2', 'stack', '--no-frame-preselection', '--path', str(capture),
                            '--out', str(out), '--device', 'cpu'], check=True, timeout=30)
            result = load_snapshot(out / 'stack.npz')
            for encoding in ('png16', 'tiff16', 'tiff32', 'tiff32_raw'):
                ...
                expected = ((result.image / min(1.43*result.image.max(),65535.)).astype(np.float32)
                            if encoding == 'tiff32' else result.image.astype(np.float32)
                            if encoding == 'tiff32_raw' else np.floor(result.image + .5).astype(np.uint16))
                if encoding == 'png16':
                    ...
                    with tifffile.TiffFile(dest) as tf:
                        ...
                        assert ('InterColorProfile' in tf.pages[0].tags) == (encoding == 'tiff32')
                        assert not parse_tiff_description(tf.pages[0].description)['result']['incomplete']
                np.testing.assert_array_equal(actual, expected)
    print('PASS: frozen mono/RGB PNG16, TIFF16, normalized/raw TIFF32 encoders and CPU SER stack')
