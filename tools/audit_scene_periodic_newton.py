"""Bounded endpoint comparison for a periodic-inverse Newton-CG update on the same scene objective."""
import argparse
import json
...
def run(path, manifest_path, directory, *, device='cpu', resume=False):
    ...
    deps = [Path(__file__), Path('tools/audit_scene_selections.py'), Path('tools/scene_study.py'),
            ...
            Path('tools/scene_projected_newton.py'), Path('tools/scene_periodic_preconditioner.py'), Path('tools/scene_retained_cache.py'),
            Path('docs/scene-periodic-newton-protocol.md'), Path('docs/scene-projected-newton-protocol.md'),
            Path('docs/scene-periodic-probe-protocol.md')]
    ...
    protocol = {'identities': identity, 'input_sha256': file_hash(path),
                ...
                'device': device, 'threads': 2, 'cache_bytes': 3*1024**3, 'headroom_bytes': 1024**3, 'preconditioner': 'periodic_uniform_sampling_inverse_v1',
                ...
                'scope': 'Periodic-inverse safeguarded Newton-CG endpoint pilot; no full-family or scientific qualification; stage resume only.',
                'q3_authorized': False}
    ...
    try:
        ...
        for selection in selected['selections']:
            ...
            batch = RetainedCellFFTBatch(ops, device=device, cache_bytes=protocol['cache_bytes'])
            ...
            inverse_started = time.monotonic()
            inverse = PeriodicScenePreconditioner(problem, workers=8)
            inverse_info = inverse.info() | {'construction_s': time.monotonic()-inverse_started}
            ...
            for cap in protocol['budgets']:
                def compute():
                    ...
                    x, info = solve(problem, max_products=cap, inner_steps=32,
                                    ...
                                    preconditioner=inverse, inner_callback=inner_progress,
            ...
            row = {'fraction': fraction, 'indices': indices, 'n_used': len(indices), 'n_captured': 500,
                   ...
                   'preconditioner': inverse_info, 'cache': batch.cache_info(), 'cpu_reference': reference_batch.cache_info(), 'wall_s': time.monotonic()-start,
