"""Bounded endpoint comparison for one alternative on the same scene objective."""
import argparse
import json
...
def run(path, manifest_path, directory, *, device='cpu', resume=False):
    ...
    deps = [Path(__file__), Path('tools/audit_scene_selections.py'), Path('tools/scene_study.py'),
            Path('tools/scene_fft.py'), Path('tools/scene_quadratic.py'), Path('tools/scene_residual.py'),
            Path('tools/scene_lbfgsb.py'), Path('docs/scene-lbfgsb-comparison-protocol.md')]
        ...
        for selection in selected['selections']:
            ...
            for cap in protocol['budgets']:
                def compute():
                    ...
                    x, info = solve(problem, maxiter=cap, tolerance=protocol['tolerance'], callback=progress,
