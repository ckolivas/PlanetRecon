...
CONTROL_HELP = {
    ...
    'local_alignment': 'Optional local seeing correction using overlapping texture patches and an averaged reference anchored to the best selected frame. Uses all colours for Bayer registration while keeping cached quality weights and original colour samples. Keeps the same selected frames and adds no sharpening. Requires cached preprocessing and Motion model none; weak or ambiguous patches keep global alignment. Experimental: the Jupiter gain is small and processing is slower.',
    'stack_percent': 'Optional reduction; default 100 uses all screened frames. Quality range: 50 keeps scores strictly above (best + worst) / 2, then applies screening. Other percentages keep the upper part of that range. Frame count: keeps this percentage of screened frames ranked by quality. Lower values trade noise for less seeing blur. Run Preprocess first; changes reuse measurements on the next run. Ignored when cached preprocessing is disabled.',
